  ...
  const Finished = () => {
      ...
      const fetchData = async () => {
        try {
          ...
          const data = Object.fromEntries(
            codes.map(code => [code, { itemCode: code, finishedQuantities: {} }])
          );
    ...
    const filteredItems = useMemo(() => {
      return itemCodes.filter(itemCode => {
        ...
        const isInStock = Object.values(itemData.finishedQuantities).some(quantity => quantity && quantity > 0);
        const isOutOfStock = Object.values(itemData.finishedQuantities).every(quantity => !quantity || quantity === 0);
      ...
      try {
        await Promise.all(
          Object.entries(itemsData).map(async ([itemCode, data]) => {
            ...
            const finishedQuantities = Object.fromEntries(
              Object.entries(data.finishedQuantities).map(([color, value]) => [color, Math.max(0, value || 0)])
            );
            await setDoc(itemRef, { itemCode, finishedQuantities }, { merge: true });
          })
        );
            ...
            {filteredItems.length > 0 ? (
              filteredItems.map((itemCode) => (
                <tr key={itemCode}>
                  ...
                  {availableColors.map((color) => {
                    const quantity = itemsData[itemCode]?.finishedQuantities?.[color] || 0;
                    return (
