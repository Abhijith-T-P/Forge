...
const Total = () => {
  ...
  const calculateTotal = (data) => {
    return data.finished.reduce((a, b) => a + b, 0) +
           data.cuttingQuantities.reduce((a, b) => a + b, 0) +
           data.tapingQuantities.reduce((a, b) => a + b, 0);
  };
        ...
        <tbody>
          {filteredItems.length > 0 ? (
            filteredItems.map(([itemCode, data]) => {
              const finishedTotal = data.finished.reduce((a, b) => a + b, 0);
              const cuttingTotal = data.cuttingQuantities.reduce((a, b) => a + b, 0);
              const tapingTotal = data.tapingQuantities.reduce((a, b) => a + b, 0);
              const total = calculateTotal(data);

              return (
                <tr key={itemCode}>
                  <td className="item-code">{itemCode}</td>
                  <td className={`quantity ${getCellClass(finishedTotal)}`}>{finishedTotal}</td>
                  <td className={`quantity ${getCellClass(cuttingTotal)}`}>{cuttingTotal}</td>
                  <td className={`quantity ${getCellClass(tapingTotal)}`}>{tapingTotal}</td>
                  <td className={`quantity total ${getCellClass(total)}`}>{total}</td>
                </tr>
              );
            })
          ) : (
            <tr>
              <td colSpan="5" className="no-item-found">No items found</td>
            </tr>
          )}
        </tbody>
      </table>
